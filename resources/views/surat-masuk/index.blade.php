            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        html += `
                        <a href="{{url('surat-masuk-edit')}}/` + data + `" class="badge bg-info">Edit</a>
...
<script>
    ...
    function updateFilesList() {
        ...
        myDropzone.files.forEach((file, index) => {
            ...
            const isSuccess = file.status === Dropzone.SUCCESS || file.serverFileId;
            html += `
                <div class="d-flex justify-content-between align-items-center p-2 mb-2 bg-white border rounded">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ${isSuccess ? `<a href="${file.path}" target="_blank" class="btn btn-sm btn-outline-primary">View</a>` : ''}
                        ${isSuccess ? '<i class="fas fa-check-circle text-success"></i>' : '<span class="spinner-border spinner-border-sm"></span>'}
                    </div>
                </div>
            `;
        });
            ...
            .then(data => {
                ...
                if (data.status === 'success' && data.files) {
                    data.files.forEach(function(f) {
                        ...
                        var mockFile = { name: name, size: size, serverFileId: f.id, path: '/storage/uploads/' + f.file };
                        ...
                        uploadedFiles.push({ name: name, size: size, path: '/storage/uploads/' + f.file, id: f.id });
                        totalFileSize += size;
                    });
                    updateFilesList();
                    existingLoaded = true;
